	...
	else
	{
		...
		else
		{
			...
			while ($allTid = mysqli_fetch_array($getAllTid4UserExec))
			{
				...
				$searchQuery="SELECT testrec.*,test.Name,test.Course_ID,test.Date,(SELECT SUM(questions.Q_Marks) AS total FROM questions JOIN test ON questions.Test_ID=test.Test_ID JOIN testrec ON test.Test_ID=testrec.Test_ID WHERE testrec.User_ID='$Uid' AND testrec.Test_ID='{$allTid['Test_ID']}')AS total FROM testrec JOIN test ON testrec.Test_ID = test.Test_ID WHERE testrec.User_ID='$Uid' AND testrec.Test_ID='{$allTid['Test_ID']}' AND test.Course_ID LIKE '$search'";
